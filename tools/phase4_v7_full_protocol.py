...
class V7FullProtocol(GatedProtocol):
    def __init__(self, env: Mapping[str, str] | None = None):
        super().__init__(env)
        self.protocol_root = Path(self.env.get("PROTOCOL_ROOT", "runs/phase4/progress1_v7_full_seed0_ready"))
    ...
    def train_command(self) -> list[str]:
        ...
        command = filtered
        command += [
            "--h6_progress_version", "P1-v7-full", "--h6_expert_enabled",
            "--h6_expert_bottleneck", "64", "--h6_expert_fofs_seed_offset", "7500",
            "--h6_expert_state_condition_scale", "0.25", "--h6_expert_scale_target", "0.10",
            "--h6_expert_scale_start_epoch", "1", "--h6_expert_scale_warmup_epochs", "6",
            "--h6_expert_max_relative_ratio", "0.10",
            "--lambda_h6_expert", "0.10", "--h6_expert_start_epoch", "1", "--h6_expert_warmup_epochs", "3",
            "--lambda_h6_advantage", "0.02", "--h6_advantage_start_epoch", "4", "--h6_advantage_warmup_epochs", "3", "--h6_advantage_margin", "0.05",
            "--lambda_h6_etf", "0.005", "--h6_etf_start_epoch", "3", "--h6_etf_warmup_epochs", "4",
            "--lambda_h6_balance", "0.01", "--lambda_h6_balance_final", "0.002", "--h6_balance_decay_epochs", "4",
            "--lambda_h6_expert_anchor", "0.01", "--h6_expert_anchor_min_cosine", "0.70",
            "--lambda_h6_expert_radius", "0.01", "--lambda_h6_orth", "0",
            "--lambda_h6_center", "0.02", "--lambda_h6_router_teacher", "0.005",
    ...
    def _marker_is_v7(self, marker: dict) -> bool:
        ...
        return payload.get("h6_config", {}).get("progress_version") == "P1-v7-full"
    ...
    def run(self) -> int:
        ...
        if not self._marker_is_v7(training): raise ProtocolError("training marker does not refer to P1-v7-full checkpoint", 3)
        ...
        write_json_atomic(self.test_dir / "EXACT_TEST_STARTED.json", {"progress_version":"P1-v7-full", "selected_epoch":epoch})
        result = subprocess.run(self.exact_test_command(epoch), env=common)
        if result.returncode: return result.returncode
        write_json_atomic(completed, {"progress_version":"P1-v7-full", "selected_epoch":epoch})
        return 0


def main() -> int:
    try: return V7FullProtocol().run()
