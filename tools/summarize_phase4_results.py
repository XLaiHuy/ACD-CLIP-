...
PATTERN = re.compile(r"^exact_results_(.+)_epoch_(\d+)\.csv$")
...
def read_exact_rows(save_path: Path, selected_epochs: set[int] | None) -> pd.DataFrame:
    rows = []
    for path in sorted(save_path.glob("exact_results_*_epoch_*.csv")):
        match = PATTERN.match(path.name)
        if match is None:
            continue
        dataset, epoch_text = match.groups()
        epoch = int(epoch_text)
        if dataset not in DATASETS or (selected_epochs is not None and epoch not in selected_epochs):
            continue
    ...
    if not rows:
        raise FileNotFoundError("no exact_results_<dataset>_epoch_<epoch>.csv files found")
    return pd.DataFrame(rows).sort_values(["epoch", "dataset"]).reset_index(drop=True)


def main():
    parser = argparse.ArgumentParser(description="Summarize six-dataset exact Phase4 test results")
    parser.add_argument("--save_path", required=True)
    parser.add_argument("--epochs", type=int, nargs="+", default=None)
    args = parser.parse_args()
    save_path = Path(args.save_path)
    rows = read_exact_rows(save_path, None if args.epochs is None else set(args.epochs))
    rows.to_csv(save_path / "results_by_dataset.csv", index=False)
    ...
    by_epoch.to_csv(save_path / "results_by_epoch.csv", index=False)
    best = by_epoch.sort_values(["combined_score", "epoch"], ascending=[False, True]).iloc[0].to_dict()
    summary = {
        "selection_scope": "cross-domain medical validation/evaluation sets",
        "datasets": DATASETS,
        "epochs": [int(value) for value in by_epoch["epoch"].tolist()],
        "macro_by_epoch": by_epoch.to_dict(orient="records"),
        "selection_rule": "maximize the six-dataset macro average of per-dataset harmonic_mean(image_score,pixel_score)",
        "best_epoch": best,
    }
    (save_path / "phase4_summary.json").write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n")
    (save_path / "best_epoch.json").write_text(json.dumps(best, indent=2, sort_keys=True) + "\n")
