...
def run_forensic(cache_root: Path, protocol_commit: str | None, implementation_commit: str | None) -> None:
    ...
    precheck_files = {name: (OUTPUT_ROOT / name).is_file() for name in output_names if name != "OUTPUT_CHECK.json"}
    json_finite = all(finite(json.loads((OUTPUT_ROOT / name).read_text())) for name in output_names if name.endswith(".json") and name != "OUTPUT_CHECK.json")
    check = {"status": "PASS", "schema_version": "P5B_FAILURE_FORENSIC_C0_OUTPUT_v1", "required_files": precheck_files, "json_finite": json_finite, "aligned_case_partition": case_taxonomy["required_counts"]["aligned_selected"] == 99037 and case_taxonomy["required_counts"]["aligned_mixed"] == 471 and case_taxonomy["required_counts"]["aligned_AN"] == 378 and case_taxonomy["required_counts"]["aligned_NA"] == 93 and case_taxonomy["required_counts"]["aligned_same_label"] == 98566, "images": main["image_count"], "classes": len(class_names), "model_forwards": 0, "training_steps": 0, "protected_source_hashes": source_hashes(), "no_dense_maps_committed": True, "no_selector_formula_change": True, "no_threshold_sweep": True}
    check["status"] = "PASS" if all(precheck_files.values()) and check["json_finite"] and check["aligned_case_partition"] and check["images"] == EXPECTED_IMAGES and check["classes"] == EXPECTED_CLASSES else "P5C0_FORENSIC_INVALID"
    write_json(OUTPUT_ROOT / "OUTPUT_CHECK.json", check)
    check["required_files"]["OUTPUT_CHECK.json"] = True
    check["output_check_written"] = True
    write_json(OUTPUT_ROOT / "OUTPUT_CHECK.json", check)
