#!/usr/bin/env python3
"""Summarize P1-v8.2 medical evaluation results for epochs 15-20 across 6 medical datasets."""
...
def compute_epoch_summary(output_dir: Path, epoch: int) -> dict[str, Any]:
    ...
    for ds in ALL_DATASETS:
        ...
        if not ds_json.is_file():
            # Try main test result csv/json if per-dataset subfolder differs
            ...
            if ds_csv.is_file():
                ...
                raise FileNotFoundError(f"Missing metric file for epoch {epoch}, dataset {ds}: {ds_json}")
        else:
...
def main():
    parser = argparse.ArgumentParser(description="Summarize P1-v8.2 medical evaluation epochs 15-20")
    parser.add_argument("--output-dir", type=Path, default=Path("runs/phase4/p1_v8_2_medical_e15_e20"))
    ...
    # Write summary CSV
    csv_path = output_dir / "medical_e15_e20_summary.csv"
    ...
    # Write summary JSON
    json_path = output_dir / "medical_e15_e20_summary.json"
    with json_path.open("w") as f:
        json.dump(summaries, f, indent=2)

    # Generate FINAL_MEDICAL_E15_E20_REPORT.md
    report_path = output_dir / "FINAL_MEDICAL_E15_E20_REPORT.md"
    with report_path.open("w") as f:
        f.write("# P1-v8.2 Six-Medical Evaluation Final Report (Epochs 15–20)\n\n")
        ...
        f.write("**Protocol Status**: `MEDICAL_E15_E20_COMPLETED`  \n\n")

        f.write("> [!IMPORTANT]\n")
        f.write("> **Epoch 20** is the canonical final checkpoint.\n")
        f.write("> Epochs 15–19 are POST-HOC DIAGNOSTIC TRAJECTORY reporting only (NOT model selection).\n\n")

        f.write("## 1. Six-Epoch Medical Summary Table\n\n")
        ...
        f.write("\n## 2. Per-Dataset Breakdown (Epochs 15–20)\n\n")
        for s in summaries:
