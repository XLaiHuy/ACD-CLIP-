#!/usr/bin/env python3
"""P5-FR1 GT-free MVTec common pass and compact all-config freeze.
...
from p5f_geometry import PACKED_GRAM_COUNT, asr, csrc, pcrr, pgm  # noqa: E402
...
def load_setup() -> dict[str, Any]:
    ...
    if setup.get("setup_status") != "PASS" or setup.get("dataset_ready") != "PASS":
        raise RuntimeError("P5FR1_PRECHECK_BLOCKED: DATASET_READY is not PASS")
    if setup.get("mask_pixels_read") or setup.get("performance_metrics_read"):
        raise RuntimeError("P5FR1_PRECHECK_BLOCKED: setup barrier metadata invalid")
    return setup
...
def load_identities() -> list[dict[str, Any]]:
    ...
    if doc.get("record_count") != EXPECTED_RECORDS or len(identities or []) != EXPECTED_RECORDS:
        raise RuntimeError("P5FR1_INPUT_PROVENANCE_INVALID: canonical MVTec count")
    if sha256_file(CANONICAL_PATH) != EXPECTED_CANONICAL_SHA:
        raise RuntimeError("P5FR1_INPUT_PROVENANCE_INVALID: canonical identity hash")
    keys = [(x["class_name"], x["image_path"]) for x in identities]
    if keys != sorted(keys) or len(set(keys)) != EXPECTED_RECORDS:
        raise RuntimeError("P5FR1_INPUT_PROVENANCE_INVALID: canonical ordering")
    return identities


def protocol_sha() -> str:
    return sha256_file(OUTPUT_ROOT / "RECOVERY_PROTOCOL.json")
...
def load_image(identity: dict[str, Any]) -> torch.Tensor:
    ...
    if "ground_truth" in relative.lower() or Path(relative).is_absolute() or ".." in Path(relative).parts:
        raise RuntimeError("P5FR1_GT_BARRIER_INVALID: unsafe image path")
    ...
    if tuple(tensor.shape) != (3, IMAGE_SIZE, IMAGE_SIZE):
        raise RuntimeError("P5FR1_INPUT_PROVENANCE_INVALID: image tensor shape")
    return tensor
...
def deploy_native_logits(native: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
    if native.ndim != 4 or tuple(native.shape[0:1] + native.shape[2:]) != (STAGES, PATCH_COUNT, 2):
        raise RuntimeError(f"P5FR1_PROTOCOL_INVALID: native logits shape={tuple(native.shape)}")
    groups = []
...
def compact_geometry(stage_features: list[torch.Tensor], peers: np.ndarray, native_margins: np.ndarray, d_rank: np.ndarray, valid: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray, float]:
    del native_margins, d_rank
    from p5f_geometry.common import GRAM_DIAG_TOL, pack_gram
    stage = torch.stack([torch.as_tensor(x).float() for x in stage_features])
    ...
    for g in range(STAGES):
        peer = stage[g][safe]
        query_parts.append((stage[g].unsqueeze(1) * peer).sum(dim=-1))
        full = torch.matmul(peer, peer.transpose(-1, -2))
        full_np = full.detach().cpu().numpy().astype(np.float32)
        diag_error = float(np.max(np.abs(np.diagonal(full_np, axis1=-2, axis2=-1) - 1.0)))
        if not np.isfinite(diag_error) or diag_error > GRAM_DIAG_TOL:
            raise RuntimeError(f"P5FR1_GEOMETRY_INVALID: Gram diagonal error {diag_error}")
        diag_max = max(diag_max, diag_error)
        gram_parts.append(pack_gram(full_np))
    query_peer = torch.stack(query_parts).detach().cpu().numpy().astype(np.float32)
    peer_gram = np.stack(gram_parts, axis=0).astype(np.float32)
    ...
    return query_peer, peer_gram, centroid.detach().cpu().numpy().astype(np.float32), diag_max
...
def construct_common(model: torch.nn.Module, image: torch.Tensor, class_name: str, text_cache: dict[str, torch.Tensor], device: torch.device) -> dict[str, np.ndarray | float | str]:
    ...
    if len(stages) != STAGES or any(tuple(feature.shape) != (PATCH_COUNT, 768) for feature in stages):
        raise RuntimeError("P5FR1_PROTOCOL_INVALID: stage feature contract")
    ...
    c, G, centroid, gram_diag_max = compact_geometry(aligned, peers, stage_margins, d_rank, valid)
    ...
    return {
        ...
        "gram_diag_max_abs": np.asarray(gram_diag_max, dtype=np.float32),
    }


def validate_record(path: Path, identity: dict[str, Any], expected_protocol: str) -> dict[str, Any]:
    with np.load(path, allow_pickle=False) as data:
        required = {"peer_indices", "valid_reference", "query_peer_cos", "peer_gram_upper", "b1_centroid_patch", "native_stage_logits", "native_stage_margins", "d_rank_patch", "deployed_score_patch", "deployed_margin_patch", "gram_diag_max_abs", "class_name"}
        if set(data.files) != required:
            raise RuntimeError(f"P5FR1_CACHE_INVALID: schema mismatch for {path.name}")
        if str(data["class_name"]) != identity["class_name"]:
            raise RuntimeError("P5FR1_CACHE_INVALID: class mismatch")
        shapes = {"peer_indices": (PATCH_COUNT, PEERS), "valid_reference": (PATCH_COUNT,), "query_peer_cos": (STAGES, PATCH_COUNT, PEERS), "peer_gram_upper": (STAGES, PATCH_COUNT, PACKED_GRAM_COUNT), "gram_diag_max_abs": (), "b1_centroid_patch": (PATCH_COUNT,), "native_stage_logits": (STAGES, PATCH_COUNT, 2), "native_stage_margins": (STAGES, PATCH_COUNT), "d_rank_patch": (PATCH_COUNT,), "deployed_score_patch": (IMAGE_SIZE * IMAGE_SIZE,), "deployed_margin_patch": (IMAGE_SIZE * IMAGE_SIZE,)}
        for key, shape in shapes.items():
            if tuple(data[key].shape) != shape:
                raise RuntimeError(f"P5FR1_CACHE_INVALID: {key} shape")
            if key != "valid_reference" and not np.all(np.isfinite(data[key])):
                raise RuntimeError(f"P5FR1_CACHE_INVALID: {key} non-finite")
...
def official_run() -> dict[str, Any]:
    ...
    if sha256_file(METADATA_PATH) != EXPECTED_METADATA_SHA or sha256_file(CHECKPOINT) != EXPECTED_CHECKPOINT_SHA or sha256_file(CONFIG) != EXPECTED_CONFIG_SHA:
        raise RuntimeError("P5FR1_INPUT_PROVENANCE_INVALID: frozen hash mismatch")
    protocol_digest = protocol_sha()
    implementation = git_head()
    ordering_hash = EXPECTED_ORDER_HASH
    CACHE_ROOT.mkdir(parents=True, exist_ok=True)
    RECORD_ROOT.mkdir(parents=True, exist_ok=True)
    if RECORD_ROOT.exists() and any(RECORD_ROOT.glob("*.npz")) and not RUN_STATUS.exists():
        raise RuntimeError("P5FR1_RUN_INVALID: stale records without run status")
    if RUN_STATUS.exists():
        state = json.loads(RUN_STATUS.read_text())
        if state.get("inflight_identity") is not None:
            raise RuntimeError("P5FR1_RUN_INVALID: unresolved inflight identity; unsafe rerun")
        if any(state.get(key) != value for key, value in (("implementation_sha", implementation), ("protocol_sha", protocol_digest), ("canonical_ordering_hash", ordering_hash))):
            raise RuntimeError("P5FR1_RUN_INVALID: resume provenance mismatch")
...
def load_configs() -> dict[str, list[dict[str, Any]]]:
    ...
    if doc.get("total_configs") != 26:
        raise RuntimeError("P5FR1_PROTOCOL_INVALID: config count")
    return doc["families"]
...
def compute_all_config_evidence() -> dict[str, Any]:
    ...
    if not RUN_STATUS.exists():
        raise RuntimeError("P5FR1_GT_FREEZE_BLOCKED: common run status missing")
    state = json.loads(RUN_STATUS.read_text())
    if state.get("state") != "FINISHED" or state.get("official_successful_forward_count") != EXPECTED_RECORDS or state.get("inflight_identity") is not None:
        raise RuntimeError("P5FR1_GT_FREEZE_BLOCKED: common run incomplete")
    ...
    if len(all_ids) != 26 or len(set(all_ids)) != 26:
        raise RuntimeError("P5FR1_PROTOCOL_INVALID: config IDs")
    record_hashes = {}
    evidence_hashes = {}
    for identity in identities:
        ...
        if not np.all(np.isfinite(evidence)):
            raise RuntimeError("P5FR1_GT_FREEZE_BLOCKED: non-finite all-config evidence")
        ...
        with np.load(target, allow_pickle=False) as out:
            if tuple(out["evidence"].shape) != (26, PATCH_COUNT) or list(out["config_ids"].astype(str)) != all_ids:
                raise RuntimeError("P5FR1_GT_FREEZE_BLOCKED: evidence schema")
...
def freeze_gt_free() -> dict[str, Any]:
    ...
    atomic_json(OUTPUT_ROOT / "COMMON/GEOMETRY_PARITY.json", {"status": "PASS", "c_shape": [3, PATCH_COUNT, PEERS], "G_shape": [3, PATCH_COUNT, PACKED_GRAM_COUNT], "gram_layout": "diag8_then_offdiag28", "gram_roundtrip": "shared pack_gram/decode_gram", "configs": 26, "reconstruction_from_c_G": True, "same_peer_ids_all_stages": True, "compact_feature_cache": False})
...
def main() -> None:
    ...
    start = time.time()
    atomic_json(COMMAND_STATUS, {"command": "audit_p5fr1_mvtec.py", "phase": args.mode, "start_time": iso_now(), "end_time": None, "elapsed_seconds": None, "exit_code": None, "completion_status": "RUNNING"})
    try:
        result = official_run() if args.mode == "official" else compute_all_config_evidence() if args.mode == "all-config" else freeze_gt_free()
        atomic_json(COMMAND_STATUS, {"command": "audit_p5fr1_mvtec.py", "phase": args.mode, "start_time": None, "end_time": iso_now(), "elapsed_seconds": time.time() - start, "exit_code": 0, "completion_status": "PASS"})
        print(json.dumps(result, indent=2, sort_keys=True))
    except Exception as exc:
        atomic_json(COMMAND_STATUS, {"command": "audit_p5fr1_mvtec.py", "phase": args.mode, "start_time": None, "end_time": iso_now(), "elapsed_seconds": time.time() - start, "exit_code": 1, "completion_status": "FAIL", "exception": repr(exc)})
        raise
