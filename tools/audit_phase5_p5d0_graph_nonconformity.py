...
def decision(analysis: dict[str, Any], graph_summary: dict[str, Any], protocol: dict[str, Any], implementation_sha: str) -> dict[str, Any]:
    ...
    return {"schema_version": "P5D0_DECISION_v1", "integrity": "PASS" if g0 else "FAIL", "model_forwards": 0, "training_steps": 0, "schema_validation": {"protocol_frozen": protocol.get("status") == "FROZEN", "cache_schema": EXPECTED_CACHE_SCHEMA, "cache_manifest_sha256": EXPECTED_CACHE_SHA, "gt_free_manifest_finalized_before_gt": True}, "b3_parity": {"aligned_certified": graph_summary["variants"]["aligned"]["edge_count"], "shifted_certified": graph_summary["variants"]["shifted"]["edge_count"], "expected_aligned_certified": 387594, "expected_shifted_certified": 1920878, "cache_only": True}, "rank_geometry": {"primary_target": "q_m(j)-q_m(i) from exact stable within-image base percentile", "all_certified_relations": True, "aligned_edges": graph_summary["variants"]["aligned"]["edge_count"], "shifted_edges": graph_summary["variants"]["shifted"]["edge_count"]}, "spatial_geometry": {"aligned": graph_summary["variants"]["aligned"]["spatial_edge_mean_distribution"], "shifted": graph_summary["variants"]["shifted"]["spatial_edge_mean_distribution"]}, "b3_protected": True, "gt_free_manifest_finalized": True, "gates": {"G0": g0, "G1": g1, "G2": g2, "G3": g3, "G4": g4, "G1_positive_mass_fraction": lev["aligned_positive_mass"]["fraction"], "G1_old_fraction": OLD_POSITIVE_FRACTION, "G1_classes_increased": lev["classes_increased_over_old"], "G2_aligned_S6_auc": auc, "G3_aligned_minus_shifted_auc": delta, "G4_mean_abs_classwise_spearman_S6_base": None if not correlations else float(np.mean(correlations))}, "candidate": "NONE", "SPATIAL_CONSTRAINT_REQUIRED": spatial_required, "terminal": terminal, "limitations": ["All GT quantities are post-hoc and not deployable.", "Graph association with masks is not causal.", "S6 is a preregistered diagnostic and no alternate signal was selected.", "Native graph leverage does not establish deployed improvement under blur, resize, stage mean, and softmax."], "assumptions": ["The immutable P5B R0 cache is the exact frozen Phase2B/R0 evidence population.", "q_m uses the audited float32 score order with ascending patch-ID ties.", "Spatial flag is descriptive and does not define a deployable distance threshold."], "exact_next_question": "If and only if the terminal is GRAPH_NONCONFORMITY_SUPPORTED_FOR_D1, can a bounded D1 use graph evidence without broad permutation or deployment mass relocation?", "forbidden_tuning_actions": protocol["forbidden"]}
