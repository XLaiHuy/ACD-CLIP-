...
- **q_m orientation:** the project’s frozen base order is descending for rank positions, while the positive transfer-safe flow requires an ascending score percentile. The corrected protocol defines `q_m=(1369-1-rank_desc)/(1369-1)` with the same float32 values and ascending patch-ID ties, so `m_i<m_j` implies `q_m(j)-q_m(i)>0`; the raw score gap remains diagnostic only.
