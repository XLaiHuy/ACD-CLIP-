...
def build_full_checkpoint(
    *,
    ...
    if hasattr(model, "nfur_refiner"):
        payload["nfur_refiner"] = {
            name: value.detach().cpu().clone()
            for name, value in model.nfur_refiner.state_dict().items()
        }
        payload["model_state"]["nfur_refiner"] = payload["nfur_refiner"]
