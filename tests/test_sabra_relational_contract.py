...
def test_relational_contract_and_feature_order():
    ...
    sensitivity = np.full(1369, 0.25, dtype=np.float32)
    record = build_relational_record(features, margins, deployment_sensitivity=sensitivity)
...
def test_relational_requires_computed_sensitivity():
    features = np.ones((3, 1369, 768), dtype=np.float32)
    margins = np.zeros((3, 1369), dtype=np.float32)
    try:
        build_relational_record(features, margins)
    except ValueError as exc:
        assert "deployment_sensitivity" in str(exc)
    else:
        raise AssertionError("missing deployment sensitivity accepted")
