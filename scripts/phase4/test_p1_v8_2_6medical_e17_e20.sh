...
# -----------------------------------------------------------------------------
# ACD-CLIP Phase 4 Progress 1 v8.2 — Dedicated 6-Medical Evaluator (Epochs 17 & 20)
# -----------------------------------------------------------------------------
...
OUTPUT_ROOT="runs/phase4/p1_v8_2_medical_e17_e20"
...
echo "=== [P1-V8.2] Dedicated 6-Medical Evaluation Plan (Epochs 17 & 20) ==="
echo "Output Root: ${OUTPUT_ROOT}"
echo "Run Directory: ${RUN_DIR}"
echo "Medical Manifest Root: ${MEDICAL_MANIFEST_ROOT}"
echo "Total Planned Dataset Evaluations: 12 (2 Epochs x 6 Datasets)"
echo "----------------------------------------------------------------------"
...
# 2. Print or Execute planned 12 evaluations
EVAL_COUNT=0
COMPLETED_COUNT=0
...
for ep in "${EPOCHS[@]}"; do
  ...
  for ds in "${DATASETS[@]}"; do
    ...
    if [ "${DRY_RUN}" -eq 1 ]; then
      echo "[EVAL ${EVAL_COUNT}/12] epoch=${ep} dataset=${ds} split=test manifest=${MANIFEST_PATH}"
      echo "  out_dir=${DS_DIR}"
      printf "  cmd: %q " "${CMD[@]}"
      echo ""
    else
      ...
      if [ -f "${DS_DIR}/metrics.json" ]; then
        echo "[SKIP] Evaluation ${EVAL_COUNT}/12 (epoch ${ep}, dataset ${ds}) already complete."
        COMPLETED_COUNT=$((COMPLETED_COUNT + 1))
        continue
      fi

      echo "=== Executing Evaluation ${EVAL_COUNT}/12 (epoch ${ep}, dataset ${ds}) ==="
...
if [ "${DRY_RUN}" -eq 1 ]; then
  echo "----------------------------------------------------------------------"
  echo "[OK] Dry-run verification complete. All 12 commands validated."
else
  echo "----------------------------------------------------------------------"
  echo "Evaluating summary across epochs 17 and 20..."
  PYTHONPATH=. python3 tools/summarize_p1_v8_2_medical_e17_e20.py --output-dir "${OUTPUT_ROOT}"
  echo "=== Dedicated 6-Medical Evaluation Pipeline Completed (${COMPLETED_COUNT}/12) ==="
fi
