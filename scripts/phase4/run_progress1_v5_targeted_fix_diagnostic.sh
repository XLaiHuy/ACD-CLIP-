...
SAVE_PATH="${SAVE_PATH:-runs/phase4/progress1_v5_targeted_fix_diagnostic_seed0}"
...
H6_LATE_FACTOR_IDENTITY_ENABLED="${H6_LATE_FACTOR_IDENTITY_ENABLED:-1}"
H6_FACTOR_ID_SCALE="${H6_FACTOR_ID_SCALE:-0.02}"
H6_FACTOR_ID_MAX_RATIO="${H6_FACTOR_ID_MAX_RATIO:-0.05}"
H6_ROUTER_TEACHER_MODE="${H6_ROUTER_TEACHER_MODE:-state_centered_cosine}"
H6_TEACHER_CONFIDENCE_GATE_ENABLED="${H6_TEACHER_CONFIDENCE_GATE_ENABLED:-1}"
H6_TEACHER_ENTROPY_THRESHOLD="${H6_TEACHER_ENTROPY_THRESHOLD:-0.98}"
H6_TEACHER_PROB_STD_THRESHOLD="${H6_TEACHER_PROB_STD_THRESHOLD:-0.001}"

IDENTITY_FLAG="--no-h6_late_factor_identity_enabled"
if [ "${H6_LATE_FACTOR_IDENTITY_ENABLED}" = "1" ]; then
  IDENTITY_FLAG="--h6_late_factor_identity_enabled"
fi
TEACHER_GATE_FLAG="--no-h6_teacher_confidence_gate"
if [ "${H6_TEACHER_CONFIDENCE_GATE_ENABLED}" = "1" ]; then
  TEACHER_GATE_FLAG="--h6_teacher_confidence_gate"
fi

echo "[PHASE4-P1-V5-FIX-DIAG] train_only=true medical_val=false medical_test=false save_path=${SAVE_PATH}"
echo "[PHASE4-P1-V5-FIX-DIAG] late_identity=${H6_LATE_FACTOR_IDENTITY_ENABLED} scale=${H6_FACTOR_ID_SCALE} max_ratio=${H6_FACTOR_ID_MAX_RATIO}"
echo "[PHASE4-P1-V5-FIX-DIAG] teacher_mode=${H6_ROUTER_TEACHER_MODE} gate=${H6_TEACHER_CONFIDENCE_GATE_ENABLED} entropy=${H6_TEACHER_ENTROPY_THRESHOLD} prob_std=${H6_TEACHER_PROB_STD_THRESHOLD}"
...
conda run --no-capture-output -n torchhuy python train.py \
  ...
  --h6_router_teacher_mode "${H6_ROUTER_TEACHER_MODE}" \
  "${TEACHER_GATE_FLAG}" \
  --h6_teacher_entropy_threshold "${H6_TEACHER_ENTROPY_THRESHOLD}" \
  --h6_teacher_prob_std_threshold "${H6_TEACHER_PROB_STD_THRESHOLD}" \
