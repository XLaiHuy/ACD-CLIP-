...
if grep -Eq -- '--(phase2b_checkpoint|load_adapter|resume|init_from_progress|freeze_phase2b)' scripts/phase4/train_progress1_v2.sh; then
  echo "The Phase4 P1-v2 train script must not initialize from an existing checkpoint." >&2
  exit 2
fi
mkdir -p "${SAVE_PATH}"
...
echo "[PHASE4-P1-V2] selected_epoch=${BEST_EPOCH} from medical validation only"
