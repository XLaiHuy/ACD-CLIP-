...
if grep -Eq -- '--(phase2b_checkpoint|load_adapter|resume|init_from_progress|freeze_phase2b)' scripts/phase4/run_progress1_v4_train.sh; then
  echo "The Phase4 P1-v4 train script must not initialize from an existing checkpoint." >&2
  exit 2
fi
mkdir -p "${SAVE_PATH}"

echo "[PHASE4-P1-V4] save_path=${SAVE_PATH} cuda=${CUDA_DEVICE} batch=${BATCH_SIZE} grad_accum=${GRAD_ACCUM} test_batch=${TEST_BATCH_SIZE} workers=${NUM_WORKERS} precision=${PRECISION} seed=${SEED} epochs=${EPOCHS}"
echo "[PHASE4-P1-V4] selection_split=val selection_rule=six_dataset_macro_combined_score validation_epochs=${VALIDATION_EPOCHS[*]}"
echo "[PHASE4-P1-V4] test_split=test test_epochs_count=1 exact_external=${EXTERNAL_EXACT_PIXEL_METRICS} chunk_pixels=${EXTERNAL_METRIC_CHUNK_PIXELS}"
...
bash scripts/phase4/run_progress1_v4_test.sh --split val "${VALIDATION_EPOCHS[@]}"
...
echo "[PHASE4-P1-V4] selected_epoch=${BEST_EPOCH} selection_split=val selection_rule=six_dataset_macro_combined_score"
echo "[PHASE4-P1-V4] test_split=test test_epochs_count=1"

SAVE_PATH="${SAVE_PATH}" CUDA_DEVICE="${CUDA_DEVICE}" TEST_BATCH_SIZE="${TEST_BATCH_SIZE}" \
NUM_WORKERS="${NUM_WORKERS}" MEDICAL_MANIFEST_ROOT="${MEDICAL_MANIFEST_ROOT}" \
H6_DENSE_ROUTING_EPOCHS="${H6_DENSE_ROUTING_EPOCHS}" H6_SPARSE_TRANSITION_EPOCHS="${H6_SPARSE_TRANSITION_EPOCHS}" \
EXTERNAL_EXACT_PIXEL_METRICS="${EXTERNAL_EXACT_PIXEL_METRICS}" EXTERNAL_METRIC_CHUNK_PIXELS="${EXTERNAL_METRIC_CHUNK_PIXELS}" \
bash scripts/phase4/run_progress1_v4_test.sh --split test "${BEST_EPOCH}"
