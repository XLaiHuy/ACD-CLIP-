...
: "${SAVE_PATH:?Set SAVE_PATH to the completed Phase4 P1-v4 training directory}"
...
for index in "${!DATASETS[@]}"; do
  ...
  echo "[PHASE4-P1-V4][${MEDICAL_SPLIT^^}][$((index + 1))/6] dataset=${DATASET} epochs=${EPOCHS[*]} exact_mode=true external_exact=${EXTERNAL_EXACT_PIXEL_METRICS} chunk_pixels=${EXTERNAL_METRIC_CHUNK_PIXELS}"
