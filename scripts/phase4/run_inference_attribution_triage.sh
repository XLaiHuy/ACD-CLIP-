...
run_ablation() {
    ...
    for i in "$@"; do
        if [[ "$i" == "--save_path" ]]; then
            # We don't have the value easily if it's the next arg, so let's do a quick regex or assume it's next.
            # Actually, we can just parse it properly.
            :
        fi
    done
    # A simpler way: just let it copy from the one we passed if it exists.
    # To be safe, we can capture the save path by reading it from the arguments list:
...
run_ablation "B0 - Phase2B baseline" \
    --h6_global_text_mode phase2b_hybrid \
    --h6_progress 0 \
    --save_path "runs/phase2b/phase2b_hybrid_alpha02_kreg2e3_lkg1e2_lr5e5_train15_test6medical7to15_fromscratch" \
    --epochs 10
...
run_ablation "A4 - optional safety comparison" \
    --h6_global_text_mode phase2b_hybrid \
    --h6_prediction_routing dense \
    --no-h6_expert_enabled
