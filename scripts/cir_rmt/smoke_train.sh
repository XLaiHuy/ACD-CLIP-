...
python -m scripts.cir_rmt.train_full --source "$SOURCE" --source-root "$SOURCE_ROOT" --clip-asset "$CLIP_ASSET" --config configs/cir_dfg_rmt_v1.json --run-root "$RUN_ROOT" --seed "${CIR_SEED:-0}" --epochs 2 --device "$DEVICE" --smoke-steps "$SMOKE_STEPS"
...
python -m scripts.cir_rmt.train_full --source "$SOURCE" --source-root "$SOURCE_ROOT" --clip-asset "$CLIP_ASSET" --config configs/cir_dfg_rmt_v1.json --run-root "$RUN_ROOT" --seed "${CIR_SEED:-0}" --epochs 3 --resume "$LAST" --device "$DEVICE" --smoke-steps 1
