...
RUN_ROOT="${RUN_ROOT:-/tmp/r3_anchor_rho_screen_20260909}"
...
test -s "${E1}"
if [[ -e "${RUN_ROOT}" ]]; then
  echo "Refusing to reuse existing rho screen root: ${RUN_ROOT}" >&2
  exit 2
fi
mkdir -p "${RUN_ROOT}"
...
for rho_tag in ${RHO_CANDIDATES}; do
  case "${rho_tag}" in
    005) rho_value="0.05" ;;
    0075) rho_value="0.075" ;;
    010) rho_value="0.10" ;;
    015) rho_value="0.15" ;;
    *) echo "unknown rho tag: ${rho_tag}" >&2; exit 2 ;;
  esac
  fork="${RUN_ROOT}/fork_${rho_tag}/adapter_1.pth"
  run="${RUN_ROOT}/rho_${rho_tag}"
  "${PY}" "${FORKER}" --source "${E1}" --output "${fork}" \
    --hybrid-alpha-max "${ALPHA}" \
    --anchor-lambda 0.0021633926715180626 --anchor-family-budget "${rho_value}" --enable-anchor
  "${PY}" "${base_args[@]}" --hybrid_alpha_max "${ALPHA}" \
    ...
    --anchor_family_budget "${rho_value}" --anchor_family_audit
  ...
  "${PY}" "${ROOT}/scripts/r3_source_tta.py" \
    ...
    --output "${RUN_ROOT}/rho_${rho_tag}_source.json" \
    --batch-size 8 --num-workers 2
done

echo "R3 rho S1 source screen complete: ${RUN_ROOT}"
