...
RUN_DIR="${RUN_DIR:-${REPO_ROOT}/runs/p1_v83_dev/corrected_300b_attempt1}"
...
if [[ -e "${RUN_DIR}" ]]; then
  echo "Refusing to reuse corrected-300B RUN_DIR: ${RUN_DIR}" >&2
  exit 2
fi

python "${REPO_ROOT}/train.py" \
  --save_path "${RUN_DIR}" \
  --dataset VisA --img_size 518 --epoch 1 --h6_smoke_max_batches 300 \
